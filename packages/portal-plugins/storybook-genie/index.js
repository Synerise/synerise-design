...
async function run() {
  await fileSelector({
    ...
    const { ext, name, base } = path.parse(file);
    ...
    try {
      ...
      mkdirSync(`../../storybook/stories-genie/${name}`, { recursive: true });
      writeFileSync(`../../storybook/stories-genie/${name}/${name}.stories${ext}`, story);

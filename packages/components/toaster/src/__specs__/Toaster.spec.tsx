...
import { removeToast, showToast } from '@synerise/ds-toast';
...
describe('Toaster', () => {
  ...
  it('toaster should render toast', async () => {
    ...
    renderWithToaster(
      <div onClick={() => showToast('success', { message: TEST_MESSAGE })}>
        {TRIGGER}
      </div>,
    );
    await waitFor(() =>
  ...
  it('toaster should render 2 toasts', async () => {
    ...
    userEvent.click(screen.getByText(TRIGGER));
    await waitFor(() =>
      expect(screen.getAllByText(TEST_MESSAGE)).toHaveLength(2),
    );
    removeToast();
  });
});

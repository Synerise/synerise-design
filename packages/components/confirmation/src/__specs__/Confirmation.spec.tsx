...
import { COLOR_TEST_CASES, DECISION_OPTIONS, ITEMS, ITEM_NAME, PROPS, RELATED_OBJECTS_LABEL } from './testData';
...
describe('Confirmation component', () => {
    it('Should render', async () => {
        renderWithProvider(
            <Confirmation {...PROPS} type='negative' open />
        );
    ...
    it.each(COLOR_TEST_CASES)('Should render correct color', async ({ type, color }) => {
        renderWithProvider(
            <Confirmation {...PROPS} type={type} open />
        );
    ...
    it('Should render decision radios', async () => {
        renderWithProvider(
            <Confirmation {...PROPS} type='negative' open decisionOptions={DECISION_OPTIONS} />
        );

        expect(await screen.findAllByRole('radio')).toHaveLength(DECISION_OPTIONS.length);
    });

    it('Should render related objects', async () => {
        const RELATED_OBJECTS = 'RELATED_OBJECTS'
        renderWithProvider(
            <Confirmation {...PROPS} type='negative' open relatedObjects={RELATED_OBJECTS} />
        );

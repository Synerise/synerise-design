...
function replaceRange(
  value: AbsoluteDateRange | RelativeDateRange,
  day: Date,
): Omit<AbsoluteDateRange, 'type'> {
  let { from, to } = value;
...
class RangePicker extends PureComponent<Props & WithDataFormatProps, State> {
  ...
  handleSideMonthChange = (
    ...
    if (fnsIsSameMonth(month, state[opposite])) {
      ...
      this.setState((prevState) => ({
        ...prevState,
        [side]: { ...state[side], adjacentMonth, mode },
      }));
      return;
    }
    ...
    return (
      <YearPicker
        ...
        onChange={(m: Date): void =>
          this.handleSideMonthChange(side, m, 'date')
        }
      />
    );
  };
  ...
  renderMonthPicker = (side: SideType): ReactNode => {
    ...
    return (
      <MonthPicker
        ...
        value={
          currentSide.month instanceof Date
            ? currentSide.month
            : new Date(currentSide.month)
        }
  ...
  renderDatePicker = (side: SideType): ReactNode => {
    ...
    const adjacentYears =
      forceAdjacentMonths ||
      fnsDifferenceInYears(ADD.MONTHS(parsedLeft, 1), parsedRight) === 0;
    return (
      <DayPicker
  ...
  renderSide = (side: SideType): ReactNode | null => {
    ...
    if (mode === MODES.TIME) {
      return this.renderTimePicker(side);
    }
        ...
        <S.PickerFooter>
          ...
          {!dateOnly && (
            ...
            >
              ...
              <Icon
                component={mode === MODES.TIME ? <CalendarM /> : <ClockM />}
              />
            </S.DateTimeModeSwitch>
          )}
        </S.PickerFooter>

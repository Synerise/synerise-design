...
- **Sub-menu nesting** — `subMenu` is fully recursive (each `SubMenu` renders items via `ItemComponent`, so a nested item with its own `subMenu` nests again); there is **no hard depth limit** and no height cap either. Indent grows 20px per level without bound.
- **Sub-menu expand/collapse animation** — items stay mounted at all times (`SubMenu.tsx` renders `dataSource` unconditionally), so the transition is pure CSS. `SubMenuContainer` is a grid whose single track goes `0fr → 1fr` (`SubMenu.styles.ts`), which animates to the sub-menu's *own* height — CSS cannot transition `height: 0 → auto`. Two invariants keep it working: `overflow: hidden` on the **container** (clips the collapsing track so nothing peeks at `0fr`), and `min-height: 0` on `SubMenuList`, the **grid item** (without it the item's automatic minimum size holds the track open at content height and the sub-menu never fully collapses). `SubMenuList` must stay the container's only child and carry no padding/border of its own. This replaced a `max-height: 999px` cap that clipped any level past ~31 default-size rows, and — since a nested container counts toward its ancestor's height — clipped much shallower trees than that. Mirrors the same pattern in `@synerise/ds-sidebar`'s `Collapse.styles.ts`.

  ...
  return (
    <InputStyles.OuterWrapper className="ds-input-group-outer-wrapper" resetMargin={resetMargin}>
      <FormField
        ...
        errorText={errors && errors.map(error => <InputStyles.ErrorText key={error}>{error}</InputStyles.ErrorText>)}
      >
        <Input.Group {...antdInputGroupProps}>
          <S.InputGroupWrapper compact={antdInputGroupProps.compact}>
            {Children.map(childrenArray, (child, index) => (
              <S.InputGroupItem className={`ds-input-group-item ds-input-group-item-${index}`}>
                {child}
              </S.InputGroupItem>
            ))}
          </S.InputGroupWrapper>

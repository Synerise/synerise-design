...
export const InputGroupWrapper = styled.div<{
  ...
  &&& {
    ...
    ${(props): FlattenSimpleInterpolation => {
      ...
      // trailing control (e.g. ColorPicker) stays compact. 'none' lets every
      // control keep its own width so nothing stretches (a compact, left-aligned
      // row of several fixed-width controls). Border-radius and the -1px border
      // overlap stay position-based (first squares its right corners, last squares
      // its left), so only the flex-grow target changes.
      const isNone = props.$growItem === 'none';
      const growChild =

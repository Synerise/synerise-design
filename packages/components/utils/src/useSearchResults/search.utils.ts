...
export const isItemInGroup = <GroupType extends BaseGroupType<GroupType>>(
  ...
  return currentGroup.subGroups
    ? currentGroup?.subGroups.findIndex((group: GroupType) => group.id === groupId) > -1
...
export const getGroupName = <GroupType extends BaseGroupType<GroupType>>(
  ...
  return groups.flatMap(group => (group.subGroups ? group.subGroups : group)).find(group => group.id === groupId)?.name;
};

...
describe('Scrollbar', () => {
  ...
  describe('overscrollBehavior', () => {
    it('should apply contain as the default on the perfect-scrollbar container', () => {
      const { container } = renderWithProvider(
        <Scrollbar maxHeight={100}>content</Scrollbar>,
      );
      const ps = container.querySelector('.ps');

      expect(ps).not.toBeNull();
      expect(ps).toHaveStyle('overscroll-behavior: contain');
    });

    it.each(['auto', 'contain', 'none'] as const)(
      'should apply the provided value (%s) on the perfect-scrollbar container',
      (value) => {
        const { container } = renderWithProvider(
          <Scrollbar maxHeight={100} overscrollBehavior={value}>
            content
          </Scrollbar>,
        );
        const ps = container.querySelector('.ps');
        ...
        expect(ps).not.toBeNull();
        expect(ps).toHaveStyle(`overscroll-behavior: ${value}`);
      },
    );
  });
});

...
describe('Tabs component', () => {
  it('should render tabs container', async () => {
    const content = <Tabs tabs={tabs} activeTab={0} handleTabClick={() => {}} />;
    renderWithProvider(content);
    const tabsContainer = screen.getByTestId('tabs-container');
    
    const tabsRendered = await within(tabsContainer).findAllByTestId('tab-container');
  ...
  it('should render dropdown', () => {
    ...
    const content = <Tabs tabs={[]} activeTab={0} handleTabClick={() => {}} configuration={{label: 'Button label', action: handleConfigurationAction}} />;
  ...
  it('should render configuration action button ', () => {
    ...
    const content = <Tabs tabs={[]} activeTab={0} handleTabClick={() => {}} configuration={{label: LABEL, action: handleConfigurationAction}} />;
    const { baseElement } = renderWithProvider(content);
  ...
  it('should render when number of tabs decreases', async function() {
    ...
    const tabsInitially = await within(tabsContainer).findAllByTestId('tab-container');
    expect(tabsInitially.length).toBe(3);
    
    rerender(<Tabs tabs={filteredTabs} activeTab={0} handleTabClick={() => {}} />);
    
    const tabsFinal = await within(tabsContainer).findAllByTestId('tab-container');

...
const MONACO_EDITOR_MENU_ID = 'EditorContext';
const MONACO_EDITOR_MENU_IDS_TO_REMOVE = [
...
const CodeArea: React.FC<CodeAreaProps> = ({
  ...
  React.useLayoutEffect(
    function resizeMonacoEditor() {
      ...
      return function resizeMonacoEditorCleanup() {
  ...
  const editorDidMountWrapper = (currentEditor: monaco.editor.IStandaloneCodeEditor, monacoInstance: typeof monaco): void => {
    editorDidMount?.(currentEditor, monacoInstance);
    setEditor(currentEditor);
  };

  const onChangeBase = (value: string, event: monaco.editor.IModelContentChangedEvent) => onChange?.(value, event);

import React, { UIEvent, useCallback, useState, forwardRef, HTMLAttributes, useEffect, MutableRefObject } from 'react';
import { ScrollbarProps } from '@synerise/ds-scrollbar/dist/Scrollbar.types';
import Scrollbar from '@synerise/ds-scrollbar';

const OuterListElement = (containerRef: MutableRefObject<HTMLDivElement | null | undefined>, isSticky: boolean) =>
  forwardRef<HTMLDivElement, HTMLAttributes<Element>>(({ onWheel, onScroll, children, style }, ref) => {
    ...
    const onWheelHandler = (event: React.WheelEvent) => {
      if (header) {
        header.scrollTo({ left: event.currentTarget?.parentElement?.scrollLeft });
      }

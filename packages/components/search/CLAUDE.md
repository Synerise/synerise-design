...
- Tests use **Vitest** — `package.json` `test` script runs `vitest run`; no `jest.config.js`. Specs use `vi.fn()`.
- The Storybook overview page lives at `packages/storybook/stories/components/Search/Search.mdx` (`<Meta title="Components/Search" />`) and covers both `Search` and `SearchInput`. Story files: `Search.stories.tsx`, `SearchInput.stories.tsx`. Each story has a `parameters.docs.source.code` snippet for the Code panel.

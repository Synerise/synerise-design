...
export const hasCustomIcon = (props: CustomIconBadgeProps | StatusIconBadgeProps): props is CustomIconBadgeProps => {
  return 'icon' in props;
};

export const renderStatusIcon = (status: StatusIconBadgeProps['status']) => {
  return STATUS_ICONS[status] ? <S.Icon status={status} {...STATUS_ICONS[status]} /> : null;
};

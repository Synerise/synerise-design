...
export const RelativeDateDropdown = ({
  ...
  const defaultTimeRelation = relativeDateValue
    ? getRelationForModifier(relativeDateValue.temporalModifier)
    : BEFORE;
  const defaultTemporalUnit =
    relativeDateValue?.temporalUnit ||
    (availableUnits?.length ? availableUnits : INTERVALS)[0];
  const defaultModifier = Math.abs(
    relativeDateValue?.temporalModifier || DEFAULT_VALUE,
  );
  ...
  const [temporalModifier, setTemporalModifier] =
    useState<number>(defaultModifier);
  ...
  useEffect(() => {
    ...
    setTemporalModifier(
      Math.abs(relativeDateValue?.temporalModifier || DEFAULT_VALUE),
    );
  }, [relativeDateValue]);
  ...
  const handleApply = () => {
    const finalModifier =
      timeRelation === BEFORE ? -temporalModifier : temporalModifier;
    isApplyEnabled &&
      onApply({ temporalModifier: finalModifier, temporalUnit });
  };

  const temporalUnitOptions = useMemo(() => {
    return (availableUnits?.length ? availableUnits : INTERVALS).map(
      (intervalOption) => (
      ...
      <S.RelativeDateDropdownWrapper data-testid="ds-factors-relative-date-dropdown">
        <InputGroup resetMargin compact>
          <InputNumber
            ...
            onChange={(newTemporalModifier) =>
              newTemporalModifier && setTemporalModifier(newTemporalModifier)
            }
          />
          <Select
            value={temporalUnit}
        ...
        <S.RelativeDateCurrentLabel>
          {texts.currentDatetime}
        </S.RelativeDateCurrentLabel>

...
describe('FormField component', () => {
    ...
    it('Should render with description', () => {
        renderWithProvider(<FormField description={DESCRIPTION}>{CHILDREN}</FormField>);
    ...
    it('Should render with label tooltip', async () => {
        renderWithProvider(<FormField label={LABEL} tooltip={TOOLTIP_TEXT}>{CHILDREN}</FormField>);
        fireEvent.mouseOver(screen.getByTestId('label-tooltip-trigger'));
        await waitFor(() => expect(screen.getByText(TOOLTIP_TEXT)).toBeInTheDocument());
    });
});

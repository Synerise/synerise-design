...
function WithMenu(menuEntryMapper = (menuEntry, idx) => menuEntry) {
  const data: MenuItemProps[] = [
    ...
    {
      text: 'Edit',
      renderHoverTooltip: () => <InformationCardWithKnobs title="Edit" subtitle="someElement.edit()">this is a custom content (editable textarea is by default)</InformationCardWithKnobs>,
    },
    {
      text: 'Duplicate',
      renderHoverTooltip: () => <InformationCardWithKnobs
        title="Duplicate"
    ...
    {
      type: 'divider',
      renderHoverTooltip: () => <InformationCard
    ...
    },
    {
      text: 'Delete',
      type: 'danger',
      renderHoverTooltip: () => <InformationCardWithKnobs title="Delete" subtitle="someElement.delete()" notice={buildExtraInfo('Note: cannot be undo', 'alert')}>{''}</InformationCardWithKnobs>,
    },
  ];
  return <>
...
function WithDropdown(numberOfElements = 1) {
  ...
  const popoverProps = React.useCallback((visible) => ({ defaultPopupVisible: dropdownVisible && (visible ?? true), }), [dropdownVisible])
  const buildMenuEntry = (visible): Partial<MenuItemProps> => ({
    text: 'Show',
    hoverTooltipProps: popoverProps(visible),
    renderHoverTooltip: () => <InformationCard title="Show" subtitle="someElement.key" descriptionConfig={{onChange: action('onChange')}}/>,
  })
  return <Dropdown

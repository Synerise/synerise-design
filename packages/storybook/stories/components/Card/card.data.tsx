...
const objectUserAvatar = (firstName = 'Jane', lastName = 'Doe') => (
  <S.ObjectWithAvatar>
    ...
    <Tooltip
      placement="top"
      title="Username with a very long and truncated label"
      trigger="hover"
      type="default"
    >
      <S.ObjectLabel
        size="small"
        ellipsis={{ tooltip: 'Username with a very long and truncated label' }}
      >
...
export const defaultRender = ({ children, ...args }: CardProps) => {
  ...
  const headerSideChildren = (
    <Button
      type="primary"
      onClick={() => updateArgs({ hideContent: !hideContent })}
    >
  ...
  return (
    <Card
      {...args}
      headerSideChildren={headerSideChildren}

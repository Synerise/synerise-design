...
export default {
  ...
  render: ({ withSelection, ...args }) => {
    ...
    const centerComponent = ({
      ...
      return (
        <Button
          type="secondary"
          mode="single-icon"
          onClick={() => updateMapping(!item.mapped, index)}
        >
    ...
    const onSelectionChange = (selected: ItemType['id'][]) =>
    ...
    const batchSelection = {
      renderCounter: (selected: number, total: number) => {
        return selected !== 0
          ? `Selected ${selected}`
          : `${mappedTotal} / ${total} parameters will be enriched`;
      },
      onSelectionChange,
      actionButtons: (
        <>
          <Button
            onClick={() => setBatchMapped(true)}
            mode="icon-label"
            type="secondary"
          >
    ...
    const leftComponent = ({ item }: { item: typeof CATALOG_ITEM_KEY }) => {
      return (
        <Input
          ...
          readOnly
          icon1={
            <Icon component={<ShowM />} onClick={action('Preview data')} />
          }
          icon1Tooltip={<>Preview data</>}
        />
      );
    };
    const rightComponent = () => {
      return (
        ...
        <Select
          ...
          onChange={(value) => {
            setEventParameter(
              EVENT_PARAMETERS.find((param) => param.id === value),
            );
          }}
        >

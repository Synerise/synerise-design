...
## Closing every overlay at once

To force-close a dropdown (and every other open DS overlay) in reaction to an app-level event, call `closeAllOverlays()` from `@synerise/ds-core`. The dropdown closes through its normal path, so `onOpenChange(false)` reaches the consumer. See the Modal docs for the full explanation and a live example.

```ts
import { closeAllOverlays } from '@synerise/ds-core';

await closeAllOverlays();                    // everything
await closeAllOverlays({ kinds: ['dropdown'] }); // dropdowns only
```

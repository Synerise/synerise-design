...
export const PopulateStep: ConditionStory = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    const contextWrapper = within(
      canvas.getByTestId('condition-subject-or-context'),
    );
    await userEvent.click(contextWrapper.getByRole('button'));
    await sleep(1500);
    await canvas.findByText(SUBJECT_ITEMS[3].name);
    await waitFor(() =>
      expect(canvas.getByText(SUBJECT_ITEMS[3].name)).not.toHaveStyle({
        pointerEvents: 'none',
      }),
    );
    ...
    await waitFor(() =>
      expect(canvas.getAllByText(PARAMETER_ITEMS[5].name)[0]).not.toHaveStyle({
        pointerEvents: 'none',
      }),
    );
    ...
    await waitFor(() =>
      expect(
        within(operatorsDropdown).getByText('Starts with'),
      ).not.toHaveStyle({ pointerEvents: 'none' }),
    );

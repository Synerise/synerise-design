...
export const PopulateFilter: FilterStory = {
  ...
  play: async ({ canvasElement, args, step }) => {
    ...
    await step('Select context', async () => {
      ...
      await waitFor(() =>
        expect(canvas.getAllByText(CONTEXT_ITEM.name)[0]).not.toHaveStyle({
          pointerEvents: 'none',
        }),
      );
    ...
    await step('Select parameter', async () => {
      await userEvent.click(
        await canvas.findByText(CONDITION_TEXTS.addFirstConditionRowButton),
      );

      await waitFor(() => expect(canvas.findAllByText(PARAMETER_ITEM.name)));
      await waitFor(() =>
        expect(canvas.getAllByText(PARAMETER_ITEM.name)[0]).not.toHaveStyle({
          pointerEvents: 'none',
        }),
      );
    ...
    await waitFor(() =>
      expect(args.onExpressionStepChange).toHaveBeenCalledTimes(4),
    );

    const operatorsDropdown = await canvas.findByTestId(
      'ds-operators-dropdown-wrapper',
    );

    await step('Select operator tab', async () => {
      const operatorsTabs =
        await within(operatorsDropdown).findAllByTestId('tab-container');

      expect(operatorsTabs).toHaveLength(5);
      await waitFor(() =>
        expect(
          within(operatorsDropdown).getAllByTestId('tab-container')[2],
    ...
    await step('Select operator', async () => {
      ...
      await waitFor(() =>
        expect(
          within(operatorsDropdown).getAllByText(OPERATOR_ITEM.name)[0],
        ).not.toHaveStyle({ pointerEvents: 'none' }),
      );
    ...
    await waitFor(() =>
      expect(args.onExpressionStepChange).toHaveBeenCalledTimes(6),
    );
    ...
    await step('Select factor type', async () => {
      ...
      await waitFor(() =>
        expect(canvas.getAllByText('Parameter')[0]).not.toHaveStyle({
          pointerEvents: 'none',
        }),
      );
    ...
    await waitFor(() =>
      expect(args.onExpressionStepChange).toHaveBeenCalledTimes(7),
    );
    ...
    await step('Select factor parameter', async () => {
      ...
      await waitFor(() =>
        expect(
          canvas.getByTestId('ds-factors-parameter-dropdown-wrapper'),
        ).toBeVisible(),
      );
      ...
      await waitFor(() =>
        expect(
          within(
            canvas.getByTestId('ds-factors-parameter-dropdown-wrapper'),
          ).getAllByText(FACTOR_ITEM.name)[0],
        ).not.toHaveStyle({ pointerEvents: 'none' }),
      );
    ...
    await waitFor(() =>
      expect(args.onExpressionStepChange).toHaveBeenCalledTimes(8),
    );
    ...
    await waitFor(() =>
      expect(canvas.getByText('Not performed')).toBeInTheDocument(),
    );
    ...
    await waitFor(() => expect(canvas.getByText('Or')).toBeInTheDocument());
  },
};

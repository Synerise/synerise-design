...
export default {
  ...
  argTypes: {
    ...
    suffixel: suffixKnobParent,
    prefixel: prefixKnobParent,
    size: {
...
export const Default: Story = {
  ...
  render: ({ setDivider, checkboxVisibilityTrigger, disabled, ordered, size, description, prefixel, suffixel, ...menuArgs}) => {
    ...
    const onClickCallback = (clickedKey: string) => {
      if (checkboxVisibilityTrigger || prefixel === prefixType.checkbox)
        return;
    ...
    const itemsWithOnClick = dataSource.map(item => {
      ...
      return {
        ...
        subMenu: item.subMenu.map(submenuItem => ({
          ...submenuItem,
          onClick: () => {
            'key' in submenuItem && onClickCallback(submenuItem.key);
          },
        }))
      };
    });
